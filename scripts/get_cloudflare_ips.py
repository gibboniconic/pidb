        ...
        if process.returncode == 0:
            ...
            if "time=" in output:
                ...
                for line in lines:
                    if "time=" in line:
                        try:
                            latency_str = line.split("time=")[1].split(" ")[0].replace("ms", "")
                            latencies.append(float(latency_str))
                        except ValueError:
                            continue
                if latencies:
                    return sum(latencies) / len(latencies)
            elif "avg = " in output: # For some Linux/macOS versions
